...
def p_DECL_ID(p):
    "DECL : INT ID"
    if (p[2] in parser.variables.keys()):
        print("Erro na linha ", parser.lines)
        print("A variavel ", p[2], " já existe")
    else:
        parser.variables.update({p[2]: parser.offset_var})
        parser.offset_var += 1
        p[0] = "PUSHI 0\n"
...
def p_ATUL_VAR(p):
    "ATUL : ID '=' MATH"
    if (p[1] not in parser.variables.keys()):
        print("Erro na linha ", parser.lines)
        print("A variavel ", p[1], " não existe")
    else:
        p[0] = p[3] + "STOREG " + str(parser.variables.get(p[1])) + "\n"
...
def p_IFBLOCK(p):
    ...
    if(p[8] == r"ELIF\d:(.|\n)+"):
        p[0] = p[3] + "JZ ELIF" + str(parser.var_elif) + "\n" + p[6] + "JUMP FIMIF" + str(parser.var_fimif) + "\n" + p[8] + "FIMIF" + str(parser.var_fimif) + ":\n"
    elif(p[8] == r"ELSE\d:(.|\n)+"):
        p[0] = p[3] + "JZ ELSE" + str(parser.var_else) + "\n" + p[6] + "JUMP FIMIF" + str(parser.var_fimif) + "\n" + p[8] + "FIMIF" + str(parser.var_fimif) + ":\n"
    else:
...
def p_IFBLOCK2_ELIF(p):
    ...
    parser.var_elif+=1
    if(p[8] == r"ELIF\d:(.|\n)+"):
        p[0] = "ELIF" + str(parser.var_elif-1) +":\n" + p[3] + "JZ ELIF" + str(parser.var_elif) + "\n" + p[6] + "JUMP FIMIF" + str(parser.var_fimif) + "\n" + p[8]
    elif(p[8] == r"ELSE\d:(.|\n)+"):
        p[0] = "ELIF" + str(parser.var_elif-1) +":\n" + p[3] + "JZ ELSE" + str(parser.var_else) + "\n" + p[6] + "JUMP FIMIF" + str(parser.var_fimif) + "\n" + p[8]
    else:
...
def p_IFBLOCK2_ELSE(p):
    ...
    parser.var_else += 1
    p[0] = "ELSE" + str(parser.var_else-1) +":\n" + p[2]
